...
export const UNPIN_ALL_EDITORS_COMMAND_ID = 'workbench.action.unpinAllEditors';
...
function registerOtherEditorCommands(): void {
	...
	CommandsRegistry.registerCommand({
		id: UNPIN_ALL_EDITORS_COMMAND_ID,
		handler: async (accessor) => {
			const editorGroupsService = accessor.get(IEditorGroupsService);
			for (const group of editorGroupsService.groups) {
				const stickyEditors = group
					.getEditors(EditorsOrder.SEQUENTIAL, { excludeSticky: false })
					.filter((editor) => group.isSticky(editor));

				// Unpin in reverse order to preserve original arrangement
				for (let i = stickyEditors.length - 1; i >= 0; i--) {
					group.unstickEditor(stickyEditors[i]);
				}
			}
		}
	});
